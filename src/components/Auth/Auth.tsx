...
import shopImage from '../../assets/images/starw.png'; 
...
const GoogleButton = (_props: ButtonProps ) => {
    ...
    const [ open, setOpen ] = useState(false) // sets state of open to open up our sign in boxes
    const [ message, setMessage ] = useState<string>() // setting the messages for our alerts
    const [ messageType, setMessageType ] = useState<MessageType>()
    const navigate = useNavigate() // instantiating useNavigate fucntion to use
    ...
    const signIn =async () => {
        ...
        onAuthStateChanged(auth, (user) => {
            ...
            if (user) {
                ...
                setMessage(`Succesfully logged in ${user.email}`)
                setMessageType('success')
                setOpen(true)
                // going to use setTimeout to display message for a short period and then navigate elsewhere
